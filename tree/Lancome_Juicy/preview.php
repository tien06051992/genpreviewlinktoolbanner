            ...
            foreach ($total[$first_key] as $key => $concept) {
                if(is_array($concept) && !empty($concept)) {
                    ...
                    ?>
                            ...
                            $pathFile = $key."/".$banner;
                            $infoFile = new SplFileInfo($pathFile);
                            if($infoFile->getExtension() == "jpg") {
                            ?>
                            <img id="i<?php echo $key."-".$key2 ?>" class="border" width="<?php echo $dimention[0]+2; ?>" height="<?php echo $dimention[1]+2; ?>" src="<?php echo $pathFile; ?>" />
                            <?php }else { ?>
                                <iframe id="i<?php echo $key."-".$key2 ?>" scrolling="no" width="<?php echo $dimention[0]+2; ?>" height="<?php echo $dimention[1]+2; ?>" src="<?php echo $pathFile; ?>"></iframe>
                            <?php } ?>
                            <hr>
                    <?php
                        } else {
                            ...
                            <?php 
                                $dimention = extract_numbers($key2); 
                                $pathFile = $key."/".$key2."/".$banner[0];
                            ?>
                            <iframe id="i<?php echo $key."-".$key2 ?>" scrolling="no" width="<?php echo $dimention[0]+2; ?>" height="<?php echo $dimention[1]+2; ?>" src="<?php echo $pathFile; ?>"></iframe>
                            <hr>
                            <?php
